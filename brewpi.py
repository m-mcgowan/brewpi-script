...
try:
    ...
    json = None                    # keep code analysis happy
    sys.exit(1)
try:
...
def oldestCompatibleBrewpiVersion():
    return "0.2.0"
...
compatibleBrewpiVersion = oldestCompatibleBrewpiVersion()


def defaultControlSettings():
    return dict(mode='b', beerSetting=20.0, fridgeSetting=20.0, heatEstimator=0.2, coolEstimator=5)


def defaultControlConstants():
    return dict(tempFormat="C", tempSetMin=1.0, tempSetMax=30.0, Kp=20.000, Ki=0.600, Kd=-3.000, iMaxErr=0.500,
                idleRangeH=1.000, idleRangeL=-1.000, heatTargetH=0.301, heatTargetL=-0.199, coolTargetH=0.199,
                coolTargetL=-0.301, maxHeatTimeForEst="600", maxCoolTimeForEst="1200", fridgeFastFilt="1",
                fridgeSlowFilt="4",
                fridgeSlopeFilt="3", beerFastFilt="3", beerSlowFilt="5", beerSlopeFilt="4", lah=0, hs=0)


def defaultControlVariables():
    return dict(beerDiff=0.000, diffIntegral=0.000, beerSlope=0.000, p=0.000, i=0.000, d=0.000, estPeak=0.000,
                negPeakEst=0.000, posPeakEst=0.000, negPeak=0.000, posPeak=0.000)

# Control Settings
cs = defaultControlSettings()

# Control Constants
cc = defaultControlConstants()

# Control variables
cv = defaultControlVariables()
...
def parseArgsAndConfig():
    global opts, args, configFile, checkDonNotRunFile, serialRestoreTimeOut, o, a, allProcesses, \
        running, config, doNotRunFilePath
    # Read in command line arguments
    try:
        opts, args = getopt.getopt(
            sys.argv[1:], "hc:sqkfd",
            ['help', 'config=', 'status', 'quit', 'kill', 'force', 'dontrunfile'])
    except getopt.GetoptError:
        print "Available Options: --help, --config <path to config file>, --quit, --kill, --force, --dontrunfile"
        sys.exit()
    configFile = None
    checkDonNotRunFile = False
    ...
    for o, a in opts:
        ...
        if o in ('-d', '--dontrunfile'):
            checkDonNotRunFile = True
    if not configFile:
        if not checkDonNotRunFile:  # Do not print when this option is active. CRON uses it and it will flood the logs
    ...
    doNotRunFilePath = config['wwwPath'] + 'do_not_run_brewpi'
    # check don't run file when it exists and exit it it does
    if checkDonNotRunFile:
        if os.path.exists(doNotRunFilePath):
...
def initializeSerial():
    global ser, port, e, dumpSerial, readAndDump, writeAndDump
    ser = 0
    # open serial port
    try:
        port = config['port']
    ...
    # yes this is monkey patching, but I don't see how to replace the methods on a dynamically instantiated
    # type any other way
    if dumpSerial:
...
def fetchVersion():
    global avrVersion, brewpiVersion
...
def createServerSocket():
    global useInetSocket, socket, port, socketFile, serialCheckInterval
    ...
    if useInetSocket:
        ...
        socket.bind((config.get('socketHost', 'localhost'), int(port)))
        ...
        socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
        socket.bind(socketFile)  # Bind BEERSOCKET
        # set all permissions for socket
        os.chmod(socketFile, 0777)
    serialCheckInterval = 0.5
    socket.setblocking(1)  # set socket functions to be blocking
    socket.listen(10)  # Create a backlog queue for up to 10 connections
    # blocking socket functions wait 'serialCheckInterval' seconds
    socket.settimeout(serialCheckInterval)
...
def handleArduinoCommand(line):
    global newData, key, newRow, csvFile, lineToWrite, e, prevDataTime, expandedMessage, lcdTextReplaced, lcdText, \
        cc, cs, cv, oldListState, serialRestoreTimeOut
    if line[0] == 'T':
        # print it to stdout
...
class SocketResult:        # enum
    (Normal, Continue, HandleSerial) = range(0, 3)


def handleSocketRequest():
    global newTemp, decoded, run, dontrunfile, newInterval, config, newName, profileUrl, profileFileName, \
        programParameters, hexFile, boardType, restoreSettings, restoreDevices, python, serialRestoreTimeOut, \
        response, configStringJson
    result = SocketResult.Normal
    ...
    if messageType == "ack":  # acknowledge request
        ...
        ser.write("s")
        result = SocketResult.HandleSerial
        ...
        run = 0
        dontrunfile = open(doNotRunFilePath, "w")
        dontrunfile.write("1")
        dontrunfile.close()
        result = SocketResult.Continue
...
while run:
    ...
    try:
        ...
        result = handleSocketRequest()
        if result == SocketResult.HandleSerial:
